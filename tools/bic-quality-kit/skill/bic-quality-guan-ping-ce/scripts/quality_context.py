...
third-party runtime dependencies. It reads local Git metadata and contained
repository files plus read-only GitHub metadata when Issue analysis is enabled.
"""
...
def discover_test_assets(
    ...
    return discover_assets(repositories, is_ignored, scan_warnings)
...
def inspect_tests(context: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    discovered = discover_test_assets(context["repositories"], scan_warnings)
...
def recommend_tests(
    ...
    correspondence = analyze_test_relations(
        WORKSPACE_ROOT, scope, symbols, tests["discovered_assets"], tests["tests"],
    )
    if tests.get("scan_warnings"):
        correspondence["scan_warnings"] = tests["scan_warnings"]
