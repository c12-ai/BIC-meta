...
# Candidate order: canonical repo name before the legacy short name, nested
# before sibling. A real git checkout wins over a bare copy so a stale non-git
# dir never shadows it.
if [ -z "${INFRA_DIR:-}" ]; then
  for _cand in "${BIC_ROOT}/BIC-infra" "${BIC_ROOT}/infra" "${BIC_ROOT}/../BIC-infra" "${BIC_ROOT}/../infra"; do
