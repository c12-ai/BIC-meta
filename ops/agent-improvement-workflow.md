...
`ready-for-human`（待 Drake 裁定）或 `ready-for-agent`（待 S3 派工）→ `stage:已实现待复测`（S3 实现）
...
- **PR 策略**：集成分支各挂一个 rolling PR（随 issue 累积、在描述里逐个勾）；
  仅高风险改动（如 D47 边界反转类）单独摘 PR 给 Drake 隔离审。
...
- agent 行为改动（prompt/中间件/图）合并前过 Drake review；graph 结构性改动先出
  方案再动手。
